...
# --- 按鈕 ---
btn_forward = swift.Button(desc="↑ Forward", cb=lambda _: move_human(0, -0.1, 0))
btn_turnL   = swift.Button(desc="⟲ Turn Left", cb=lambda _: move_human(0, 0, math.pi/8))
btn_turnR   = swift.Button(desc="⟳ Turn Right", cb=lambda _: move_human(0, 0, -math.pi/8))
...
def moving_wall_collision(human, env):
    ...
    while True:
        ...
        if not hit and e_stop:
            e_stop = False
            print("✅ Human clear, E-STOP released.")
...
def RMRC_lift():  
    global e_stop, holding, target_ball, trash_offset, robot, env  # 🔹全域變數宣告
